...
app.post(apiversion + "/auth/register", (req, res) => {
  ...
  try {
    db.query(
      `INSERT INTO users (username,password,role) VALUES ( '${user.username}','${hashedPassword}','${user.role}');`,
...
app.post(apiversion + "/order", verify, function (req, res) {
  try {
    ...
    let mapDetails = JSON.parse(details);
    ...
    db.query(
      ...
      VALUES ( ${userId},'${name}', '${address}', ${total});`,
      function (error, results, fields) {
        if (error) throw error;
        db.query(
          "SELECT orderId as orderId FROM orders ORDER BY orderId DESC LIMIT 1",
          function (error, results, fields) {
            if (error) throw error;
            var orderId = Number(
              JSON.parse(JSON.stringify(results))[0]["orderId"]
            );

            mapDetails.forEach((item) => {
              db.query(
                `INSERT INTO orderdetail (orderId, bookid, price, qty) VALUES(${orderId},${item.bookId}, ${item.price}, ${item.qty});`,
                function (error, results, fields) {
                  if (error) throw error;
                }
              );
